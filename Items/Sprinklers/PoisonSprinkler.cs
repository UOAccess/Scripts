            ...
            else
            {
                if (this.IsAccessibleTo(from))
                {
                    if (m_UsesRemaining < 1)
                        from.SendMessage("You must add poison potions to this before you can use it!");
                    else
                    {
            ...
            if (item is GreaterPoisonPotion && m_UsesRemaining < m_StorageLimit)
            {
                ...
                if (keg.Type == PotionEffect.PoisonGreater && keg.Held <= freespace )
                    ...
                    if (keg.Type == PotionEffect.PoisonGreater && keg.Held >= freespace)
                    {
                        ...
                        InvalidateProperties();
                        from.PlaySound(0x240);
                        from.SendMessage("You place the keg back in your backpack."); // You place the empty bottle in your backpack.
                        Container pack = from.Backpack;
                        pack.DropItem(keg);
                        return true;
                }
